...
class Fwi(object):
    ...
    def __init__(self, model, dx, dt, train_dataset, dev_dataset, propagator,
                 ...
                 #optimizer=tf.train.GradientDescentOptimizer(learning_rate=lr).minimize(loss6,global_step=global_step),
                 batch_size=1,
                 l2_regularizer_scale=0.0,
                 autodiff=True, save_gradient=False):
    
        ndim = model.ndim
        train_sources = train_dataset[0]#震源数据
        train_receivers = train_dataset[2]#接收
        ...
        regularizer = \
                tf.keras.regularizers.l2(l2_regularizer_scale)#L2正则化
        #model = tf.get_variable('model', initializer=tf.constant(model),
                           #     regularizer=regularizer)#根据原始model初始化model
        model = tf.Variable(model, dtype=tf.float32)
        batch_placeholders = _create_batch_placeholders(ndim,
                                                        num_time_steps,
                                                        num_sources_per_shot,
                                                        num_receivers_per_shot)#储存每一个批量数据

        out_wavefields = propagator(model, batch_placeholders['sources'],
                                    batch_placeholders['sources_x'], dx, dt)#计算u

        out_receivers = _extract_receivers(out_wavefields,
                                           batch_placeholders['receivers_x'],
                                           num_receivers_per_shot)#挑选u


        reg_ws = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)#获取正则化损失

        regularization_loss = tf.reduce_sum(reg_ws)#对所有损失求和
        loss1 = tf.losses.mean_squared_error(10000*batch_placeholders['receivers'],
                                             10000*out_receivers)#计算总损失
        #loss2=tf.abs(tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels=batch_placeholders['receivers'],logits=out_receivers)))
        loss3=tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels=batch_placeholders['receivers'],
                                             logits=out_receivers,dim=0))
   
        loss4=tf.losses.absolute_difference(10000*batch_placeholders['receivers'],
                                             10000*out_receivers)#计算总损失
        loss5=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=batch_placeholders['receivers'],
                                             logits=out_receivers))
        loss6=tf.reduce_mean(tf.keras.losses.logcosh(10000*batch_placeholders['receivers'],10000*out_receivers))
        
        loss7=tf.reduce_sum(tf.keras.losses.logcosh(100*batch_placeholders['receivers'],100*out_receivers))
        loss = loss6+regularization_loss
...
def _get_dev_loss(model_shape, dev_dataset, batch_placeholders, loss, sess,
    ...
    dev_batch_size = 1 # Must be a divisor of the dev dataset size
...
def _prepare_batch(step, dataset, model_shape, batch_size, batch_placeholders):
    ...
    ndim = len(model_shape)#维数
    num_sources_per_shot = int(sources.shape[2])#震源个数
    num_receivers_per_shot = int(receivers.shape[2])#接收个数
    num_shots = int(sources.shape[1])#几次炮点
    ...
    src_batch = sources[:, batch_start : batch_end, :]#震源小批量
    ...
    src_x_batch = np.zeros([batch_size, num_sources_per_shot, ndim+1],
                           np.int)#初始化sources_x,并添加一个维度
    src_x_batch[:, :, 1:] = sources_x[batch_start : batch_end, :, :]#将震源坐标存入
    src_x_batch[:, :, 0] = \
            np.tile(np.arange(batch_size).reshape([-1, 1]),
                    [1, num_sources_per_shot])#给batch_size里的样本标号0，1，...,batch_size-1

    rec_batch = receivers[:, batch_start : batch_end, :]#接收点小批量
